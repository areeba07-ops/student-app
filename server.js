...
app.post("/teacher-register", (req, res) => {
  ...
  db.query("SELECT id FROM teacher_accounts WHERE email = ? OR username = ?", [email, username], (err, existing) => {
    ...
    bcrypt.hash(password, 10, (hashErr, hash) => {
      ...
      db.query(
        "INSERT INTO teacher_accounts (name, email, username, password, teacher_id) VALUES (?, ?, ?, ?, ?)",
...
app.get("/teacher-profile", verifyTeacherToken, (req, res) => {
  db.query(
    `SELECT ta.id, ta.name, ta.email, ta.username, ta.teacher_id, t.course, t.name AS fullName
     FROM teacher_accounts ta LEFT JOIN teachers t ON ta.teacher_id = t.id WHERE ta.id = ?`,
    [req.teacher.id],
    (err, result) => {
      if (err || !result || result.length === 0)
...
app.post("/student-register", (req, res) => {
  ...
  if (!name || !email || !username || !password || !studentId)
    return res.json({ success: false, message: "All fields required." });
  db.query("SELECT id FROM student_accounts WHERE email = ? OR username = ?", [email, username], (err, existing) => {
    if (err) return res.json({ success: false, message: "Server error." });
    if (existing && existing.length > 0)
      return res.json({ success: false, message: "Email or username already taken." });
    bcrypt.hash(password, 10, (hashErr, hash) => {
      if (hashErr) return res.json({ success: false, message: "Error securing password." });
      db.query(
        "INSERT INTO student_accounts (name, email, username, password, student_id) VALUES (?, ?, ?, ?, ?)",
...
app.put("/students/:id", verifyToken, (req, res) => {
  const { name, email, course } = req.body;
  db.query("UPDATE students SET name=?, email=?, course=? WHERE id=?", [name, email, course, req.params.id], (err) => {
    res.json({ success: !err });
  });
});

/* STUDENTS DELETE — cascades all related data */
app.delete("/students/:id", verifyToken, (req, res) => {
  const id = req.params.id;
  db.query("DELETE FROM teacher_attendance WHERE student_id = ?", [id], () => {
    db.query("DELETE FROM teacher_grades WHERE student_id = ?", [id], () => {
      db.query("DELETE FROM enrollments WHERE student_id = ?", [id], () => {
        db.query("DELETE FROM student_accounts WHERE student_id = ?", [id], () => {
...
app.get("/students/search/:key", verifyToken, (req, res) => {
  ...
  db.query("SELECT * FROM students WHERE name LIKE ? OR email LIKE ? OR course LIKE ?", [key, key, key], (err, result) => res.json(result || []));
});
...
app.post("/teachers", verifyToken, (req, res) => {
  const { name, email, course } = req.body;
  if (!name || !email || !course) return res.json({ success: false });
  db.query("INSERT INTO teachers (name, email, course) VALUES (?, ?, ?)", [name, email, course], (err) => res.json({ success: !err }));
});

app.put("/teachers/:id", verifyToken, (req, res) => {
  const { name, email, course } = req.body;
  db.query("UPDATE teachers SET name=?, email=?, course=? WHERE id=?", [name, email, course, req.params.id], (err) => res.json({ success: !err }));
});

/* TEACHERS DELETE — cascades all related data */
app.delete("/teachers/:id", verifyToken, (req, res) => {
  const id = req.params.id;
  db.query("DELETE FROM timetabless WHERE teacher_id = ?", [id], () => {
    db.query("DELETE FROM allocations WHERE teacher_id = ?", [id], () => {
      db.query("DELETE FROM teacher_assignments WHERE teacher_id = ?", [id], () => {
        db.query("DELETE FROM teacher_materials WHERE teacher_id = ?", [id], () => {
          db.query("DELETE FROM teacher_announcements WHERE teacher_id = ?", [id], () => {
            db.query("DELETE FROM teacher_accounts WHERE teacher_id = ?", [id], () => {
...
app.post("/courses", verifyToken, (req, res) => {
  ...
  db.query("INSERT INTO courses (name, instructor, duration) VALUES (?, ?, ?)", [name, instructor, duration], (err) => res.json({ success: !err }));
});

app.put("/courses/:id", verifyToken, (req, res) => {
  const { name, instructor, duration } = req.body;
  db.query("UPDATE courses SET name=?, instructor=?, duration=? WHERE id=?", [name, instructor, duration, req.params.id], (err) => res.json({ success: !err }));
});

/* COURSES DELETE — cascades all related data */
app.delete("/courses/:id", verifyToken, (req, res) => {
  const id = req.params.id;
  db.query("DELETE FROM timetabless WHERE course_id = ?", [id], () => {
    db.query("DELETE FROM allocations WHERE course_id = ?", [id], () => {
      db.query("DELETE FROM teacher_attendance WHERE course_id = ?", [id], () => {
        db.query("DELETE FROM teacher_grades WHERE course_id = ?", [id], () => {
          db.query("DELETE FROM teacher_assignments WHERE course_id = ?", [id], () => {
            db.query("DELETE FROM teacher_materials WHERE course_id = ?", [id], () => {
              db.query("DELETE FROM teacher_announcements WHERE course_id = ?", [id], () => {
...
app.post("/departments", verifyToken, (req, res) => {
  ...
  db.query("INSERT INTO departments (name, program, duration) VALUES (?, ?, ?)", [name, program, duration], (err) => res.json({ success: !err }));
});

app.put("/departments/:id", verifyToken, (req, res) => {
  const { name, program, duration } = req.body;
  db.query("UPDATE departments SET name=?, program=?, duration=? WHERE id=?", [name, program, duration, req.params.id], (err) => res.json({ success: !err }));
});

app.delete("/departments/:id", verifyToken, (req, res) => {
  db.query("DELETE FROM departments WHERE id=?", [req.params.id], (err) => res.json({ success: !err }));
});
...
app.post("/enrollments", verifyToken, (req, res) => {
  ...
  db.query("SELECT id FROM enrollments WHERE student_id=? AND course_id=?", [studentId, courseId], (err, existing) => {
    if (err) return res.json({ success: false });
    if (existing.length > 0) return res.json({ success: false, message: "Already enrolled" });
    db.query("INSERT INTO enrollments (student_id, course_id) VALUES (?, ?)", [studentId, courseId], (err2) => res.json({ success: !err2 }));
  });
});
...
app.put("/enrollments/:id", verifyToken, (req, res) => {
  const { studentId, courseId } = req.body;
  if (!studentId || !courseId) return res.json({ success: false });
  db.query("SELECT id FROM enrollments WHERE student_id=? AND course_id=? AND id<>?", [studentId, courseId, req.params.id], (err, existing) => {
    if (err) return res.json({ success: false });
    if (existing.length > 0) return res.json({ success: false, message: "Already enrolled" });
    db.query("UPDATE enrollments SET student_id=?, course_id=? WHERE id=?", [studentId, courseId, req.params.id], (err2) => res.json({ success: !err2 }));
  });
});

/* ENROLLMENTS DELETE — also removes attendance and grades for that student+course */
app.delete("/enrollments/:id", verifyToken, (req, res) => {
  const id = req.params.id;
  db.query("SELECT student_id, course_id FROM enrollments WHERE id = ?", [id], (err, rows) => {
    if (err || !rows || rows.length === 0)
      return res.json({ success: false, message: "Enrollment not found" });
    const { student_id, course_id } = rows[0];
    db.query("DELETE FROM teacher_attendance WHERE student_id = ? AND course_id = ?", [student_id, course_id], () => {
      db.query("DELETE FROM teacher_grades WHERE student_id = ? AND course_id = ?", [student_id, course_id], () => {
        db.query("DELETE FROM enrollments WHERE id = ?", [id], (err2) => {
...
app.post("/allocations", verifyToken, (req, res) => {
  ...
  db.query("SELECT id FROM allocations WHERE teacher_id=? AND course_id=?", [teacherId, courseId], (err, existing) => {
    if (err) return res.json({ success: false });
    if (existing.length > 0) return res.json({ success: false, message: "Already allocated" });
    db.query("INSERT INTO allocations (teacher_id, course_id) VALUES (?, ?)", [teacherId, courseId], (err2) => res.json({ success: !err2 }));
  });
});
...
app.put("/allocations/:id", verifyToken, (req, res) => {
  const { teacherId, courseId } = req.body;
  if (!teacherId || !courseId) return res.json({ success: false });
  db.query("SELECT id FROM allocations WHERE teacher_id=? AND course_id=? AND id<>?", [teacherId, courseId, req.params.id], (err, existing) => {
    if (err) return res.json({ success: false });
    if (existing.length > 0) return res.json({ success: false, message: "Already allocated" });
    db.query("UPDATE allocations SET teacher_id=?, course_id=? WHERE id=?", [teacherId, courseId, req.params.id], (err2) => res.json({ success: !err2 }));
  });
});

/* ALLOCATIONS DELETE — also removes timetable for that teacher+course */
app.delete("/allocations/:id", verifyToken, (req, res) => {
  const id = req.params.id;
  db.query("SELECT teacher_id, course_id FROM allocations WHERE id = ?", [id], (err, rows) => {
    if (err || !rows || rows.length === 0)
      return res.json({ success: false, message: "Allocation not found" });
    const { teacher_id, course_id } = rows[0];
    db.query("DELETE FROM timetabless WHERE teacher_id = ? AND course_id = ?", [teacher_id, course_id], () => {
      db.query("DELETE FROM allocations WHERE id = ?", [id], (err2) => {
        res.json({ success: !err2 });
      });
    });
  });
...
app.post("/timetabless", verifyToken, (req, res) => {
  ...
  db.query("SELECT id FROM timetabless WHERE semester=? AND day=? AND time=? AND teacher_id=?", [semester, day, time, teacherId], (err, existing) => {
    if (err) return res.json({ success: false });
    if (existing.length > 0) return res.json({ success: false, message: "Time slot already occupied" });
    db.query("INSERT INTO timetabless (semester, day, time, room, teacher_id, course_id) VALUES (?, ?, ?, ?, ?, ?)", [semester, day, time, room, teacherId, courseId], (err2) => res.json({ success: !err2 }));
  });
});
...
app.put("/timetabless/:id", verifyToken, (req, res) => {
  const { semester, day, time, room, teacherId, courseId } = req.body;
  if (!semester || !day || !time || !teacherId || !courseId) return res.json({ success: false });
  db.query("SELECT id FROM timetabless WHERE semester=? AND day=? AND time=? AND teacher_id=? AND id<>?", [semester, day, time, teacherId, req.params.id], (err, existing) => {
    if (err) return res.json({ success: false });
    if (existing.length > 0) return res.json({ success: false, message: "Time slot already occupied" });
    db.query("UPDATE timetabless SET semester=?, day=?, time=?, room=?, teacher_id=?, course_id=? WHERE id=?", [semester, day, time, room, teacherId, courseId, req.params.id], (err2) => res.json({ success: !err2 }));
  });
});

app.delete("/timetabless/:id", verifyToken, (req, res) => {
  db.query("DELETE FROM timetabless WHERE id=?", [req.params.id], (err) => res.json({ success: !err }));
});
...
app.post("/semesters", verifyToken, (req, res) => {
  ...
  db.query(
    "INSERT INTO semesters (name, start_date, end_date, reg_deadline, status) VALUES (?, ?, ?, ?, ?)",
...
app.put("/semesters/:id", verifyToken, (req, res) => {
  ...
  if (!name) return res.json({ success: false, message: "Name required" });
  db.query(
    "UPDATE semesters SET name=?, start_date=?, end_date=?, reg_deadline=?, status=? WHERE id=?",
...
app.delete("/semesters/:id", verifyToken, (req, res) => {
  db.query("DELETE FROM semesters WHERE id=?", [req.params.id], (err) => res.json({ success: !err }));
});
...
app.post("/grade-boundaries", verifyToken, (req, res) => {
  ...
  db.query(
    "INSERT INTO grade_boundaries (grade, min_percent, max_percent, gpa_points, remark) VALUES (?, ?, ?, ?, ?)",
...
app.put("/grade-boundaries/:id", verifyToken, (req, res) => {
  const { grade, min, max, gpa, remark } = req.body;
  db.query(
    "UPDATE grade_boundaries SET grade=?, min_percent=?, max_percent=?, gpa_points=?, remark=? WHERE id=?",
...
app.delete("/grade-boundaries/:id", verifyToken, (req, res) => {
  db.query("DELETE FROM grade_boundaries WHERE id=?", [req.params.id], (err) => res.json({ success: !err }));
});
...
app.post("/grade-boundaries/replace-all", verifyToken, (req, res) => {
  ...
  db.query("DELETE FROM grade_boundaries", (err) => {
    ...
    const values = boundaries.map(b => [b.grade || "", b.min || 0, b.max || 0, b.gpa || 0, b.remark || ""]);
    db.query(
      "INSERT INTO grade_boundaries (grade, min_percent, max_percent, gpa_points, remark) VALUES ?",
      [values],
...
function getTeacherId(accountId, callback) {
  db.query("SELECT teacher_id FROM teacher_accounts WHERE id = ?", [accountId], (err, rows) => {
  ...
  getTeacherId(req.teacher.id, (teacherId) => {
    ...
    db.query(
      `SELECT tt.id, tt.semester, tt.day, tt.time, tt.room,
       ...
       LEFT JOIN courses c ON tt.course_id = c.id
       WHERE tt.teacher_id = ?
       ORDER BY FIELD(tt.day,'Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'), tt.time`,
      [teacherId], (err, result) => res.json(result || [])
    );
  });
});
...
app.post("/teacher/attendance", verifyTeacherToken, (req, res) => {
  ...
  db.query("DELETE FROM teacher_attendance WHERE course_id = ? AND date = ?", [courseId, date], (err) => {
    if (err) return res.json({ success: false });
    const values = records.map(r => [courseId, r.studentId, date, r.status || "present"]);
    db.query(
      "INSERT INTO teacher_attendance (course_id, student_id, date, status) VALUES ?",
      [values], (err2) => res.json({ success: !err2 })
    );
  });
});
  ...
  getTeacherId(req.teacher.id, (teacherId) => {
    ...
    db.query(
      `SELECT ta.*, s.name AS studentName, c.name AS courseName
       ...
       JOIN allocations a ON a.course_id = ta.course_id AND a.teacher_id = ?
       ORDER BY ta.date DESC`,
      [teacherId], (err, result) => res.json(result || [])
    );
  });
...
app.post("/teacher/grades", verifyTeacherToken, (req, res) => {
  ...
  db.query(
    `INSERT INTO teacher_grades (course_id, student_id, midterm, final, assignment)
     VALUES (?, ?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE midterm=VALUES(midterm), final=VALUES(final), assignment=VALUES(assignment)`,
...
app.post("/teacher/assignments", verifyTeacherToken, (req, res) => {
  ...
  getTeacherId(req.teacher.id, (teacherId) => {
    ...
    db.query(
      "INSERT INTO teacher_assignments (teacher_id, course_id, title, description, deadline, max_marks) VALUES (?, ?, ?, ?, ?, ?)",
...
app.delete("/teacher/assignments/:id", verifyTeacherToken, (req, res) => {
  db.query("DELETE FROM teacher_assignments WHERE id = ?", [req.params.id], (err) => res.json({ success: !err }));
});
...
app.post("/teacher/materials", verifyTeacherToken, (req, res) => {
  ...
  getTeacherId(req.teacher.id, (teacherId) => {
    if (!teacherId) return res.json({ success: false });
    db.query(
      "INSERT INTO teacher_materials (teacher_id, course_id, title, type, url) VALUES (?, ?, ?, ?, ?)",
...
app.delete("/teacher/materials/:id", verifyTeacherToken, (req, res) => {
  db.query("DELETE FROM teacher_materials WHERE id = ?", [req.params.id], (err) => res.json({ success: !err }));
});
...
app.post("/teacher/announcements", verifyTeacherToken, (req, res) => {
  ...
  getTeacherId(req.teacher.id, (teacherId) => {
    if (!teacherId) return res.json({ success: false });
    db.query(
      "INSERT INTO teacher_announcements (teacher_id, course_id, title, message) VALUES (?, ?, ?, ?)",
...
app.delete("/teacher/announcements/:id", verifyTeacherToken, (req, res) => {
  db.query("DELETE FROM teacher_announcements WHERE id = ?", [req.params.id], (err) => res.json({ success: !err }));
});

/* ===============================================================
   STUDENT DASHBOARD ROUTES (FIXED)
   =============================================================== */

function getStudentId(accountId, callback) {
  db.query("SELECT student_id FROM student_accounts WHERE id = ?", [accountId], (err, rows) => {
    if (err || !rows || rows.length === 0) return callback(null);
